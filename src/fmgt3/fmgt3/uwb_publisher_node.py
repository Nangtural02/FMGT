# uwb_tracker/uwb_publisher_node.py
import rclpy
import serial
import re
import threading
    ...
    def run(self):
        ...
        try:
            while self.is_running and rclpy.ok():
                try:
                    ...
                    if self.ser is None:
                        ...
                        self._logger.info(f"{self.port} 연결 성공.")
                    ...
                    if self.is_running:
                        self._logger.warn(f"{self.port} 연결 끊김. 2초 후 재시도...")
                        time.sleep(2)
                
                except (ValueError, AttributeError) as e:
                    self._logger.warn(f"{self.port}에서 데이터 파싱 오류: {e}")
                    continue
            ...
            if self.ser and self.ser.is_open:
                self.ser.close()
                self._logger.info(f"{self.port} 연결이 정상적으로 종료되었습니다.")
...
class UwbPublisherNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('UWB Publisher Node 시작됨.')

    def timer_callback(self):
        msg = Point()
        msg.x = self.reader_a.latest_distance_m
        msg.y = self.reader_b.latest_distance_m
        self.publisher_.publish(msg)

    def on_shutdown(self):
        self.get_logger().info('UWB Publisher 종료 중...')
        self.reader_a.stop()
        self.reader_b.stop()
